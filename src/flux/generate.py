...
def generate(
    ...
    conditions_texture: List[Condition] = None,
    ...
    **params: dict,
    ...
    if use_condition:
        ...
        for condition in conditions:
            # if condition.position_delta==None:
    ...
    if use_texture_image:
        condition_latents_texture, condition_ids_texture, condition_type_ids_texture = (
            ([] for _ in range(3))
        )
        for condition in conditions_texture:
            tokens, ids, type_id = condition.encode(self)
            condition_latents_texture.append(tokens)  # [batch_size, token_n, token_dim]
            condition_ids_texture.append(ids)  # [token_n, id_dim(3)]
        condition_latents_texture = torch.cat(condition_latents_texture, dim=1)
        condition_latents_texture_mask = conditions_texture_mask
    ###################
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            noise_pred = tranformer_forward(
                ...
                condition_latents_texture=condition_latents_texture if use_texture_image else None,
