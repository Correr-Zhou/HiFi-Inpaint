...
def attn_forward(
    ...
    condition_latents_texture: torch.FloatTensor = None,
    condition_latents_texture_mask: torch.FloatTensor = None,
    attention_mask: Optional[torch.FloatTensor] = None,
    image_rotary_emb: Optional[torch.Tensor] = None,
    cond_rotary_emb: Optional[torch.Tensor] = None,
    model_config: Optional[Dict[str, Any]] = {},
    use_texture_image=False,
    alpha=None,
) -> torch.FloatTensor:
    batch_size, _, _ = (
        hidden_states.shape
    ...
    if use_texture_image:  # 新增纹理图像条件判断
        ...
        cond_texture_query = cond_texture_query.view(batch_size, -1, attn.heads, head_dim).transpose(
            1, 2
        )
        cond_texture_key = cond_texture_key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
        cond_texture_value = cond_texture_value.view(batch_size, -1, attn.heads, head_dim).transpose(
            1, 2
        )
        if attn.norm_q is not None:
            cond_texture_query = attn.norm_q(cond_texture_query)
        if attn.norm_k is not None:
            cond_texture_key = attn.norm_k(cond_texture_key)
        if cond_rotary_emb is not None:
            cond_texture_query = apply_rotary_emb(cond_texture_query, cond_rotary_emb)
            cond_texture_key = apply_rotary_emb(cond_texture_key, cond_rotary_emb)
        if condition_latents is not None:
            texture_query = torch.cat([query, cond_texture_query], dim=2)
            texture_key = torch.cat([key, cond_texture_key], dim=2)
            texture_value = torch.cat([value, cond_texture_value], dim=2)
        hidden_states_texture = F.scaled_dot_product_attention(texture_query, texture_key, texture_value,dropout_p=0.0, is_causal=False, attn_mask=attention_mask)
    ...
    if encoder_hidden_states is not None:
        if condition_latents is not None:
            ...
            if use_texture_image:
                ...
                hidden_states_texture = hidden_states_texture.to(query.dtype)
                _,hidden_states_texture,_ = (
                hidden_states_texture[:, : encoder_hidden_states.shape[1]],
                hidden_states_texture[
                    :, encoder_hidden_states.shape[1] : -condition_latents.shape[1]
                ],
...
def block_forward(
    ...
    condition_latents_texture: torch.FloatTensor,
    condition_latents_texture_mask: torch.FloatTensor,
    ...
    ######modify#####
    if use_texture_image:
        if condition_latents_texture is not None:
            (
                norm_condition_latents_texture,
                texture_gate_msa,
                texture_shift_mlp,
                texture_scale_mlp,
                texture_gate_mlp,
            ) = self.norm1(condition_latents_texture, emb=cond_temb)
    ...
    result = attn_forward(
        ...
        condition_latents_texture=norm_condition_latents_texture if use_texture_image else None,
        condition_latents_texture_mask=condition_latents_texture_mask if use_texture_image else None,
        image_rotary_emb=image_rotary_emb,
        cond_rotary_emb=cond_rotary_emb if use_cond else None,
        use_texture_image=use_texture_image,
        alpha=self.alpha if use_texture_image else None,
    )
